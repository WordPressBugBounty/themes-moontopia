...
function moontopia_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'moontopia_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function moontopia_theme() {
	return Theme::getInstance();
}

function moontopia_assets() {
	return moontopia_theme()->getAssetsManager();
}
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	moontopia_assets()
		...
		->registerStylesheet( 'moontopia-theme', '/theme/theme.css' );
	moontopia_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		moontopia_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'moontopia_print_color_scheme', 0 );
} else {
	moontopia_assets()
		->registerStyle( 'moontopia-theme', moontopia_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function moontopia_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'moontopia_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'moontopia_after_activation_redirect_url' );
function moontopia_after_activation_redirect_url( $url ) {
...
function moontopia_get_builder_plugin_slug() {
...
function moontopia_theme_plugins( $plugins ) {

	$plugin_slug = moontopia_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'moontopia' ) : __( 'Kubio', 'moontopia' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'moontopia_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === moontopia_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'moontopia' ),
									moontopia_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'moontopia' ),
									moontopia_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'moontopia' ) ); ?>
                        </a>
...
function moontopia_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'moontopia_try_online_url', 10 );

function moontopia_render_header_style() {

	?>
    <base target="_top">
	<?php

	moontopia_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'moontopia_render_header_style', 100 );
