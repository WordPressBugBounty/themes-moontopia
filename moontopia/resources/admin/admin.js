...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".moontopia-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.moontopia_admin) {
    return;
  }

  var _window$moontopia_adm = window.moontopia_admin,
      builderStatusData = _window$moontopia_adm.builderStatusData,
      getStartedData = _window$moontopia_adm.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.moontopia-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".moontopia-admin-big-notice").addClass("processing");
      $(".moontopia-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".moontopia-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.moontopia-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".moontopia-admin-big-notice");
  $custom_close_button = $notice_root.find(".moontopia-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".moontopia-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.moontopia_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
